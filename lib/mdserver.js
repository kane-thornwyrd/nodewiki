...
function withMdFile(directory, file, name, cb){
  fs.readFile(directory + name, 'utf-8', function(err, data){
    if (err){
      ...
      cb({fileContents: markdownParsed, error: {error: false}, rawMd: data, fileName: file.name });
    }
  });
}
...
function sendFile(file, directory, socket){
  // withMeta(directory, function(meta){
    ...
    if (!allowedExtensions.isMarkdown(name)) {
      // something other then a markdown file was requested for
      socket.emit('openReply', {fileContents: "", error: {error: true, reason: "file requested is not a markdown file"}});
    } else {
      withMdFile(directory, file, name, function (data) {
        socket.emit('openReply', data);
      });
    }
  // });
}
...
function saveFile(file, directory, socket){
  // withMeta(directory, function(meta){
    if (allowedExtensions.isMarkdown(file.name) && typeof file.content != 'undefined'){
